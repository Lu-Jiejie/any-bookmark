...
起因：在 ~~yande.re~~ 上搜图的时候想存几个 tag 组合，发现这网站没有收藏功能。使用浏览器收藏又太重，而且不好管理与打开，我要做的只是“在页面中管理页面相关的收藏”。
...
在添加栏里给当前域名设一个正则表达式，比如 `^(.+?)\s*\|\s*Example.com$`，之后点"正则提取原标题"就会自动把页面标题 `Foo | Example.com` 提取为 `Foo`.
...
自动同步：每 5 分钟自动检查远端更新，本地增删收藏 3 秒内推送到远端。
