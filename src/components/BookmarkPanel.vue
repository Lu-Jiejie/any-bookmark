...
const hasNewName = computed(() => newName.value.trim().length > 0 && newUrl.value.trim().length > 0)

const domainRegex = computed(() => getDomainRegex(hostname.value))

// Edit via browser prompt
function startEdit(index: number, name: string, url: string) {
  ...
  if (nextName.trim() && nextUrl.trim()) {
    ...
    updateBookmark(realIndex, nextName.trim(), nextUrl.trim())
  }
}
...
<template>
  ...
  >
    ...
    >
      ...
      <div
        max-w="[95vw]" max-h="[90vh]" flex="~ col"
        border="1 solid border"
        rounded-2xl bg-panel w-130 relative z-1 overflow-x-hidden overflow-y-auto
        p="x-4 y-3"
          ...
          >
            ...
            <div flex="~ col gap-1.5">
              ...
              <input
                ...
                font-mono outline-none rounded-lg transition-200
                @keydown.enter="handleAdd()"
              >
            </div>
            <div flex="~ col gap-1.5">
              <span text="xs white/30">正则表达式</span>
              <input
                :value="domainRegex || ''"
                type="text"
            ...
            <div flex="~ col gap-1" p-1 border="1 solid border rounded-lg">
              ...
              >
                ...
                <button
                  ...
                  title="编辑名称与地址"
                  @click="startEdit(i, bm.name, bm.url)"
                >
